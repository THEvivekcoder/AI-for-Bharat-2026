/**
 * BharatSahayak API Client
 * Centralized API communication with error handling, retry logic, and authentication
 */
...
class BharatSahayakAPI {
    ...
    async register(userData) {
        const result = await this.post('/auth/register', userData, { skipAuth: true });
        return result;
    }

    async login(phoneNumber) {
        const result = await this.post('/auth/login', { phone_number: phoneNumber }, { skipAuth: true });
        return result;
    }

    async verifyOTP(phoneNumber, otp, session) {
        const result = await this.post('/auth/verify', {
